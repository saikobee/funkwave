...
DAT_IMG = pyglet.resource.image("stupid-dude.png")
#DAT_IMG = pyglet.resource.image("ness.png")
...
def mirror(sprite):
    sprite.tex_shape.left,  sprite.tex_shape.right = \
    sprite.tex_shape.right, sprite.tex_shape.left
...
class MainWindow(pyglet.window.Window):
    ...
    def update(self, dt):
        ...
        for sprite in self.sprites:
            vals = [0.00, sprite.lol_w/2.0]
            #vals = [0.00]
            n = len(vals)
            #sprite.u = vals[random.randrange(n)]
            #sprite.u = vals[self.tot % n]
            sprite.u = vals[int(self.time * 4) % n]
            #sprite.u = vals[0]
