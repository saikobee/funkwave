...
file =  os.path.join("alpha", "alpha8x.png")
alphabet_img = pygame.image.load(file).convert_alpha()

def random_letter():
    #alphabet = "abcdefghijklmnopqrstuvwxyz"
    alphabet = "abc"
    i = random.randrange(len(alphabet))
    letter = alphabet[i]

    char_size = 64

    left,  top    = char_size*i, 0
    width, height = char_size,   char_size
    rect = pygame.Rect(left, top, width, height)

    img  = alphabet_img.subsurface(rect)

    star = rabbyt.Sprite(surface_to_texture_id(img))
